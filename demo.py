...
from SurfMatch.Detection import detectShow
import cv2
class picture(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("传统目标检测效果演示")
    ...
    def detect(self):
        ...
        if picture.path1!="" and picture.path2 !="":
            xmin, ymin, xmax, ymax, h2 ,w2 = detectShow(picture.path1, picture.path2)
            #xMin = xmin*((self.label2.width()/w2))
            #yMin = ymin * ((self.label2.height() / h2))
            #xMax =xmin + (xmax-xmin)*((self.label2.width()/w2))
            #yMax = ymin + (ymax - ymin) * ((self.label2.height() / h2))
            print(xmin, ymin, xmax, ymax)
            self.jpg = cv2.imread(picture.path2)
            if xmin != -1:
                self.cvRGBImg = cv2.rectangle(self.jpg, (int(xmin*0.95), int(ymin*0.9)), (int(xmax*1.05), int(ymax*1.1)), (0, 0, 255), 10)
            else:
                self.cvRGBImg = cv2.putText(self.jpg, "no logo detected", (50, 50), cv2.FONT_HERSHEY_COMPLEX, 1.0, (100, 50, 200), 3)
